...
class Crawler(object):
    def login(self, url):
        ...
        if captcha:
            ...
            if r.status_code == 200:
                print('Login Success !')
                self.get_short_comments(r.text)
            else:
    ...
    def get_page_txt(self, url):
        s = requests.Session()
        r = s.get(cons.url_login, headers=cons.headers)
        self.get_short_comments(r.text)
    ...
    def get_short_comments(self, page_txt):
        ...
        if index != len(user_id_list) \
            ...
            _db = db.DB()
            _db.save_to_csv(dict_list)
            next_url = tree.xpath('//div[@id="paginator"]/a/@href')
            if next_url != None:
                self.get_page_txt(cons.url_cmts_2[0] + next_url)
            else:
